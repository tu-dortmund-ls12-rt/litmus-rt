...
static void psfp_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{
	...
	TRACE_TASK(t, "P-SFP: task new, num_segments = %d, first segment = $d",
			t->rt_param.task_params.num_segments,
			t->rt_param.task_params.exec_costs[0]);
...
#endif
	now = litmus_clock();
	// TODO: Inferred job releases should probably only happen if the job is currently in its last segment
	// with similar reasoning as the semaphore case below
